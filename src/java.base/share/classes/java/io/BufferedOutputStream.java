...
public class BufferedOutputStream extends FilterOutputStream {
    ...
    private void implWrite(@PolySigned int b) throws IOException {
        growIfNeeded(1);
        if (count >= buf.length) {
            flushBuffer();
        }
    ...
    private void implWrite(@PolySigned byte[] b, @IndexOrHigh({"#1"}) int off, @LTLengthOf(value={"#1"}, offset={"#2 - 1"}) @NonNegative int len) throws IOException {
        if (len >= maxBufSize) {
