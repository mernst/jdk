    ...
    implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
     ...
     */
    public synchronized void setSize(@GuardSatisfied @Modifiable @Shrinkable Vector<E> this, @NonNegative int newSize) {
        modCount++;
     ...
     *         ({@code index < 0 || index >= size()})
     */
    public synchronized void removeElementAt(@GuardSatisfied @Modifiable @Shrinkable Vector<E> this, @NonNegative int index) {
        if (index >= elementCount) {
            throw new ArrayIndexOutOfBoundsException(index + " >= " +
                                                     elementCount);
        }
     ...
     */
    public synchronized boolean removeElement(@GuardSatisfied @Modifiable @Shrinkable Vector<E> this, Object obj) {
        modCount++;
        int i = indexOf(obj);
        if (i >= 0) {
            removeElementAt(i);
     ...
     * method (which is part of the {@link List} interface).
     */
    public synchronized void removeAllElements(@GuardSatisfied @Modifiable @Shrinkable Vector<E> this) {
        final Object[] es = elementData;
     ...
     * @since 1.2
     */
    public boolean remove(@GuardSatisfied @Modifiable @Shrinkable Vector<E> this, @GuardSatisfied @Nullable @UnknownSignedness Object o) {
     ...
     * @since 1.2
     */
    public synchronized E remove(@GuardSatisfied @Modifiable @Shrinkable Vector<E> this, @NonNegative int index) {
        modCount++;
        if (index >= elementCount)
            throw new ArrayIndexOutOfBoundsException(index);
        E oldValue = elementData(index);
     ...
     * @since 1.2
     */
    public void clear(@GuardSatisfied @Modifiable @Shrinkable Vector<E> this) {
        removeAllElements();
    }
     ...
     * @since 1.2
     */
    public boolean removeAll(@GuardSatisfied @Modifiable @Shrinkable Vector<E> this, Collection<? extends @UnknownSignedness Object> c) {
     ...
     * @since 1.2
     */
    public boolean retainAll(@GuardSatisfied @Modifiable @Shrinkable Vector<E> this, Collection<? extends @UnknownSignedness Object> c) {
        Objects.requireNonNull(c);
        return bulkRemove(e -> !c.contains(e));
    }
     ...
     */
    protected synchronized void removeRange(@GuardSatisfied @Modifiable @Shrinkable Vector<E> this, int fromIndex, int toIndex) {
        modCount++;
