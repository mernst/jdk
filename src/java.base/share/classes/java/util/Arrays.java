...
public class Arrays {
     ...
     *         runtime type that can be stored in the specified array
     */
    public static void fill(@PolyInterned @PolyMustCall @PolyNull @PolySigned Object @MustCallUnknown [] a, @IndexOrHigh({"#1"}) int fromIndex, @IndexOrHigh({"#1"}) int toIndex, @PolyInterned @PolyMustCall @PolyNull @PolySigned Object val) {
        rangeCheck(a.length, fromIndex, toIndex);
        for (int i = fromIndex; i < toIndex; i++)
            a[i] = val;
    }
